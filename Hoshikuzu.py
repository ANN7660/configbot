...
async def on_ready():
    ...
    bot.start_time = datetime.datetime.now(datetime.timezone.utc)  # ⬅️ AJOUTE ÇA
    log_action(f"✅ Bot connecté: {bot.user}")
...
# Clé API pour sécuriser (même que dans le panel)
API_KEY = "TON_CLE_SECRETE_123"  # ⬅️ CHANGE ICI

# Liste des logs (stockage en mémoire)
bot_logs = []
...
def log_action(action):
    ...
    if len(bot_logs) > 1000:  # Garde seulement les 1000 derniers logs
        bot_logs.pop(0)
    print(log_entry)
...
@app.before_request
def check_api_key():
    # protège les routes POST/PUT/DELETE par la clé API
...
def home():
    return jsonify({
        "status": "online",
        "message": "Hoshikuzu API is running",
        "bot": str(bot.user) if hasattr(bot, 'user') and bot.is_ready() else "Not ready"
    })
...
def api_status():
    ...
    uptime = datetime.datetime.now(datetime.timezone.utc) - bot.start_time if hasattr(bot, 'start_time') else None
...
def api_guilds():
    ...
    log_action(f"📋 Liste des serveurs récupérée ({len(guilds)} serveurs)")
    return jsonify(guilds)

@app.route('/api/guilds/<int:guild_id>/members')
def api_guild_members(guild_id):
    ...
    members = []
    for m in list(guild.members)[:100]:  # Limite à 100 membres
        members.append({
            "id": m.id,
            "name": str(m),
            "bot": m.bot
    ...
    log_action(f"👥 Membres de {guild.name} récupérés ({len(members)} membres)")
    return jsonify(members)

@app.route('/api/guilds/<int:guild_id>/leave', methods=['POST'])
def api_leave_guild(guild_id):
    """Quitter un serveur"""
    guild = bot.get_guild(guild_id)
    if not guild:
        return jsonify({"error": "Guild not found"}), 404
    ...
    bot.loop.create_task(leave())
    log_action(f"🚪 Quitté le serveur: {guild_name}")
    return jsonify({"status": "left", "guild": guild_name})

@app.route('/api/guilds/<int:guild_id>/members/<int:member_id>/kick', methods=['POST'])
def api_kick_member(guild_id, member_id):
    """Kick un membre"""
    guild = bot.get_guild(guild_id)
    if not guild:
        return jsonify({"error": "Guild not found"}), 404
    ...
    async def kick():
        await member.kick(reason="Kicked from remote panel")

    bot.loop.create_task(kick())
    log_action(f"🚫 Membre expulsé: {member_name} de {guild.name}")
    return jsonify({"status": "kicked", "member": member_name})

@app.route('/api/guilds/<int:guild_id>/members/<int:member_id>/ban', methods=['POST'])
def api_ban_member(guild_id, member_id):
    """Ban un membre"""
    guild = bot.get_guild(guild_id)
    if not guild:
        return jsonify({"error": "Guild not found"}), 404
    ...
    async def ban():
        await member.ban(reason="Banned from remote panel")

    bot.loop.create_task(ban())
    log_action(f"🔨 Membre banni: {member_name} de {guild.name}")
    return jsonify({"status": "banned", "member": member_name})
...
def api_config():
    ...
    if request.method == 'GET':
        ...
        return jsonify({
            "prefix": "+",
            "status": "hoshikuzu | +help",
            "color": "#764ba2"
        })
    else:
        # Sauvegarde la config
        config = request.json
        log_action(f"⚙️ Configuration mise à jour: {config}")
...
def api_sync():
    ...
    async def sync():
        await bot.tree.sync()
        log_action("🔄 Commandes slash synchronisées")
...
def api_clear_cache():
    """Vider le cache"""
    log_action("🗑️ Cache vidé")
    # Ajoute ton code pour vider le cache ici
    return jsonify({"status": "cleared"})

@app.route('/api/announcement', methods=['POST'])
def api_announcement():
    """Envoie une annonce globale"""
    ...
    async def send_announcement():
        ...
        for guild in bot.guilds:
            try:
                # Trouve le premier channel où le bot peut écrire
                for channel in guild.text_channels:
                    if channel.permissions_for(guild.me).send_messages:
                        await channel.send(f"📢 **ANNONCE**\n{message}")
                        sent += 1
                        break
            except:
                pass
        log_action(f"📢 Annonce envoyée à {sent} serveurs")

    bot.loop.create_task(send_announcement())
    return jsonify({"status": "sent", "message": message})

@app.route('/api/backup', methods=['POST'])
def api_backup():
    """Créer un backup"""
    log_action("💾 Backup créé")
    # Ajoute ton code pour créer un backup ici
    return jsonify({"status": "backed_up"})

@app.route('/api/restart', methods=['POST'])
def api_restart():
    """Redémarre le bot (sur Render)"""
    log_action("🔄 Redémarrage du bot...")

    def restart():
        import time
        time.sleep(1)
        os._exit(0)  # Render redémarrera automatiquement le bot
...
def api_logs():
    """Récupère les logs"""
    return jsonify({
        "logs": "\n".join(bot_logs[-100:]) if bot_logs else "Aucun log"
    })
...
def start_api():
    """Démarre l'API Flask"""
    port = int(os.environ.get("PORT", 8080))
    log_action(f"🌐 API démarrée sur le port {port}")
